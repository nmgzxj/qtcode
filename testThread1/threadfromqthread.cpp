...
{
    msleep(500);
    QString str = QString("%1->%2,thread id:%3").arg(__FUNCTION__).arg(__FILE__).arg((unsigned int)(size_t)QThread::currentThreadId());
    emit message(str);
}
...
{
    msleep(500);
    emit message(QString("%1->%2,thread id:%3").arg(__FUNCTION__).arg(__FILE__).arg((unsigned int)(size_t)QThread::currentThreadId()));
}
...
void ThreadFromQThread::run()
{
    int count = 0;
    m_isCanRun = true;
    QString str = QString("%1->%2,thread id:%3").arg(__FILE__).arg(__FUNCTION__).arg((unsigned int)(size_t)QThread::currentThreadId());
    emit message(str);
    while(1)
    {
        sleep(1);
